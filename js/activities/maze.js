...
const MazeActivity = {
    ...
    showInstructions(callback) {
        ...
        text.innerHTML = `
            ...
            • Find glowing orbs along the way to reveal memories<br><br>
    ...
    generateMaze() {
        ...
        // Create corner entrances for players
        // Player 1 entrance: TOP-LEFT corner
        this.mazeGrid[0][0].walls.top = false;
        this.mazeGrid[0][0].walls.left = false;

        // Player 2 entrance: BOTTOM-RIGHT corner (for duet mode)
        this.mazeGrid[rows - 1][cols - 1].walls.bottom = false;
        this.mazeGrid[rows - 1][cols - 1].walls.right = false;

        // Clear a 3x3 area in the center for the goal heart
        ...
        for (let dr = -1; dr <= 1; dr++) {
            for (let dc = -1; dc <= 1; dc++) {
                ...
                if (r >= 0 && r < rows && c >= 0 && c < cols) {
                    // Remove internal walls in the center area
                    if (dr !== 1) this.mazeGrid[r][c].walls.bottom = false;
                    if (dr !== -1) this.mazeGrid[r][c].walls.top = false;
                    if (dc !== 1) this.mazeGrid[r][c].walls.right = false;
                    if (dc !== -1) this.mazeGrid[r][c].walls.left = false;
                }
            }
        }

        // NO extra openings - keep the maze challenging!
    ...
    enterMaze() {
        ...
        // Position players at OPPOSITE CORNERS of the maze
        const rows = this.mazeRows;
        const cols = this.mazeCols;

        // Player 1 starts at TOP-LEFT corner
        this.player.x = this.mazeOffsetX + this.cellSize / 2;
        this.player.y = this.mazeOffsetY + this.cellSize / 2;
        ...
        if (this.mode === 'duet' && this.player2) {
            ...
            this.player2.y = this.mazeOffsetY + (rows - 0.5) * this.cellSize;
        }
